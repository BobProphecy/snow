...
order_customer_info AS (
  ...
  {#Compiles customer information along with their order details for better insights.#}
  SELECT 
...
Filter_1 AS (
  ...
  FROM customer_order_summary AS in0
...
total_sales_summary AS (
  ...
  FROM Filter_1 AS customer_order_summary
...
DEMO_IMPEL AS (
  ...
  FROM {{ source('BOBW.DEMO_OUTPUT', 'DEMO_IMPEL') }}
